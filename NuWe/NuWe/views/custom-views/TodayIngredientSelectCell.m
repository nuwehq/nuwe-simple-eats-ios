...
@implementation TodayIngredientSelectCellScrollView
...
@interface TodayIngredientSelectCell ()  <UIScrollViewDelegate, UITextFieldDelegate>
{
    CGFloat _height;
...
@property (nonatomic, strong) TodayIngredientSelectCellScrollView *cellScrollView;
...
@implementation TodayIngredientSelectCell
...
- (void)initializer
{
    ...
    self.cellScrollView = [[TodayIngredientSelectCellScrollView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.tableview.frame), _height)];
...
- (void)onSelectLevel:(UIButton*)button
{
    if (button.selected == YES)
    {
        button.selected = NO;
        _textAmount.text = @"";
        [self.delegate updateIngredientWithId:self.mainIngredient.szId withAmount:[NSNumber numberWithInt:0]];
    }
    else
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            if (index == button.tag)
            {
                ...
                int nAmount = (int)[gData getDefaultIngrededientAmount:nLevel ingredient:self.mainIngredient];
                _textAmount.text = [NSString stringWithFormat:@"%d", nAmount];
                [self.delegate updateIngredientWithId:self.mainIngredient.szId withAmount:[NSNumber numberWithInt:nAmount]];
            }
            else
                btn.selected = NO;
        }
...
- (void)updateLevel:(int)nAmount update:(BOOL)fUpdate
{
    ...
    int nLevel = [gData getIngredientLevelFromAmount:nAmount ingredient:_mainIngredient];
...
- (void)updateCellWithIngredient:(IngredientSubGroup*)ingredient withAmount:(int)amount update:(BOOL)fUpdate
{
    ...
    self.lblName.text = ingredient.szName;
    [self updateLevel:amount update:fUpdate];
}
