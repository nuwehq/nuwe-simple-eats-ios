...
{
    NSLog(@"index path row -- %d", (int)indexPath.row);
...
- (void)expandCell:(NSIndexPath*)indexPath
{
    if ([_SKSTableViewDelegate respondsToSelector:@selector(numberOfSectionsInTableView:)])
    {
        ...
        NSInteger numberOfSections = [_SKSTableViewDelegate numberOfSectionsInTableView:self];
...
- (void)updatePathInfo
{
    ...
    NSMutableDictionary* expandableCells = [NSMutableDictionary dictionary];
    for (NSString* key in [_expandableCells allKeys])
    {
        id expanded = [_expandableCells objectForKey:key];
        ...
        [expandableCells setObject:expanded forKey:newIndexPath];
    }
    ...
    {
        for (int j = 0; j < [_expandedIndexPaths[i] count]; j++)
        {
            indexPath = [_expandedIndexPaths[i] objectAtIndex:j];
            newRow = indexPath.row;
            if (newRow > 0)
                newRow++;

            NSIndexPath* newIndexPath = [NSIndexPath indexPathForRow:newRow inSection:indexPath.section];
            [_expandedIndexPaths[i] replaceObjectAtIndex:j withObject:newIndexPath];
        }
    }
...
- (void)insertExpandedIndexPaths:(NSArray *)indexPaths forSection:(NSInteger)section
{
    ...
    if ([self.expandedIndexPaths[section] count] > 0)
        firstIndexPathExpanded = self.expandedIndexPaths[section][0];
...
- (void)accessoryViewAnimationForCell:(SKSTableViewCell *)cell
{
    ...
    __block UIImageView* _toggleView = (UIImageView*)[cell viewWithTag:500];
    ...
    [UIView animateWithDuration:0.2 animations:^{
        if (_cell.isExpanded) {
            ...
            _toggleView.transform = CGAffineTransformMakeRotation(M_PI);
            
        } else {
            _toggleView.transform = CGAffineTransformMakeRotation(0);
        }
