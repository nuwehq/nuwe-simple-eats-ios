...
- (void)onSelectLevel:(UIButton*)button
{
    ...
    else
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            {
                btn.selected = YES;
                int nLevel = button.tag - 50 + 1;
                
                int nAmount = [gData getDefaultIngrededientAmount:nLevel index:_nCellIndex];
...
- (void)updateLevel:(int)nAmount update:(BOOL)fUpdate
{
    ...
    int nLevel = [gData getIngredientLevelFromAmount:nAmount index:_nCellIndex];
